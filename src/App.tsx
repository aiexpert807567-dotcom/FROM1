...
export default function App() {
  ...
  useEffect(() => {
    ...
    const performSeek = (time: number) => {
      ...
      // Safety watchdog: 60ms timeout
      if (isSeeking && now - lastSeekTime > 60) {
        isSeeking = false;
      }
      ...
      // 6ms micro-throttle (~166 seeks/sec)
      if (now - lastSeekTime < 6) {
        latestTargetTime = time;
        return;
      }
    ...
    // ── PHASE 1: Stream immediately so first frame shows ASAP ──
    video.preload = "auto";
    video.src = videoUrl;
    video.load();
    ...
    const onCanPlay = () => {
      // First frame decoded — update status immediately
    ...
    // ── PHASE 2: Blob cache in background (uses fetch with high priority hint) ──
    ...
    fetch(videoUrl, {
      ...
      .catch((err) => {
        if (err.name !== "AbortError") {
          console.warn("Blob cache failed, streaming:", err);
          setVideoStatus("● Live stream active");
        }
      });
    ...
    const renderTick = (now: number) => {
      ...
      // ── SMOOTH VIDEO SCRUB ──
      // Use fixed lerp factor independent of frame rate for predictable smoothness
      if (vidDur > 0) {
        const diff = targetTime - scrubTime;
        // 0.55 lerp = faster response, less sticky than original 0.45
        scrubTime += diff * 0.55;
        scrubTime = Math.max(0, Math.min(scrubTime, vidDur));
        if (Math.abs(video.currentTime - scrubTime) > 0.001) {
          performSeek(scrubTime);
        }
      }

      // ── SMOOTHER SPIDER BODY TRACKING ──
      // Use dt-compensated lerp so it's consistent at any frame rate
      // Higher factor = tighter tracking, less lag
      ...
      ctx.beginPath();
      ctx.moveTo(0, 9); ctx.quadraticCurveTo(4, 14, 0, 20); ctx.quadraticCurveTo(-4, 14, 0, 9); ctx.fill();
  ...
  return (
    <>
      {/* All font sizes increased ~10% from original */}
      <style>{`
        ...
        * { margin:0; padding:0; box-sizing:border-box; cursor: none !important; }
        ...
        /* Watch Trailer btn: was 11.55px → 12.7px */
        #watch-btn {
        ...
        /* Title: was clamp(84px,11.55vw,155.4px) → clamp(92.4px,12.7vw,170.9px) */
        #title-main {
          font-family: 'Murder DB', 'Cinzel', serif;
        ...
        /* Tagline: was clamp(14.7px,1.26vw,17.85px) → clamp(16.2px,1.39vw,19.6px) */
        #tagline {
        ...
        /* Hero buttons: was 11.55px → 12.7px */
        .hero-btn {
          font-family: 'Murder DB', 'Cinzel', serif;
          font-size:12.7px; letter-spacing:4px; text-transform:uppercase;
        ...
        /* Modal close: was 10.5px → 11.55px */
        #modal-close {
        ...
        /* Modal section h3: was 10.5px → 11.55px */
        ...
        /* Timeline date: was 10.5px → 11.55px */
        ...
        /* Timeline event: was 14.7px → 16.2px */
        ...
        /* Trailer close: was 11.55px → 12.7px */
        .trailer-close {
